...
trait SitacModel extends org.sitac.SitacContainer {
		...
		private lazy val typeEntrees : scala.collection.mutable.ListBuffer[org.sitac.TypeEntree] = new scala.collection.mutable.ListBuffer[org.sitac.TypeEntree]()
...
if(this.interventionTypes!= interventionTypes){
		...
		def getTypeEntrees : List[org.sitac.TypeEntree] = {
				typeEntrees.toList
		}
		def getTypeEntreesForJ : java.util.List[org.sitac.TypeEntree] = {
				import scala.collection.JavaConversions._
				typeEntrees
		}

		def setTypeEntrees(typeEntrees : List[org.sitac.TypeEntree] ) {
if(this.typeEntrees!= typeEntrees){
				this.typeEntrees.clear()
				this.typeEntrees.insertAll(0,typeEntrees)
				typeEntrees.foreach{el=>
					el.setEContainer(this,Some(()=>{this.removeTypeEntrees(el)}))
				}
}

		}

		def addTypeEntrees(typeEntrees : org.sitac.TypeEntree) {
				typeEntrees.setEContainer(this,Some(()=>{this.removeTypeEntrees(typeEntrees)}))
				this.typeEntrees.append(typeEntrees)
		}

		def addAllTypeEntrees(typeEntrees : List[org.sitac.TypeEntree]) {
				typeEntrees.foreach{ elem => addTypeEntrees(elem)}
		}

		def removeTypeEntrees(typeEntrees : org.sitac.TypeEntree) {
				if(this.typeEntrees.size != 0 && this.typeEntrees.indexOf(typeEntrees) != -1 ) {
						this.typeEntrees.remove(this.typeEntrees.indexOf(typeEntrees))
						typeEntrees.setEContainer(null,None)
				}
		}

		def removeAllTypeEntrees() {
				this.typeEntrees.foreach{ elem => removeTypeEntrees(elem)}
		}
...
def getClonelazy(subResult : java.util.IdentityHashMap[Object,Object]): Unit = {
		...
		this.getTypeEntrees.foreach{ sub => 
			sub.getClonelazy(subResult)
		}
...
def resolve(addrs : java.util.IdentityHashMap[Object,Object]) : SitacModel = {
		...
		this.getTypeEntrees.foreach{sub =>
			clonedSelfObject.addTypeEntrees(addrs.get(sub).asInstanceOf[org.sitac.TypeEntree])
		}
