...
public class TDPuller extends TDReplicator implements TDChangeTrackerClient {
    ...
    public void beginReplicating() {
        ...
        logger.warn(TDDatabase.TAG, this + " starting ChangeTracker with since=" + lastSequence);
    ...
    public void changeTrackerReceivedChange(Map<String, Object> change) {
        ...
        if(!TDDatabase.isValidDocumentId(docID)) {
            logger.warn(TDDatabase.TAG, String.format("%s: Received invalid doc ID from _changes: %s", this, change));
            return;
        }
    ...
    public void changeTrackerStopped(TDChangeTracker tracker) {
        logger.warn(TDDatabase.TAG, this + ": ChangeTracker stopped");
...
//        }
    ...
    public void processInbox(TDRevisionList inbox) {
        ...
        //logger.warn(TDDatabase.TAG, String.format("%s: Looking up %s", this, inbox));
        ...
        if(!db.findMissingRevisions(inbox)) {
            logger.warn(TDDatabase.TAG, String.format("%s failed to look up local revs", this));
            inbox = null;
        }
        ...
        if(inboxCount == 0) {
            ...
            logger.warn(TDDatabase.TAG, String.format("%s no new remote revisions to fetch", this));
            setLastSequence(lastInboxSequence);
            return;
        }

        logger.debug(TDDatabase.TAG, this + " fetching " + inboxCount + " remote revisions...");
        //logger.debug(TDDatabase.TAG, String.format("%s fetching remote revisions %s", this, inbox));
            ...
            public void onCompletion(Object result, Throwable e) {
                ...
                if(result != null) {
                    ...
                    if(history != null) {
                        ...
                        logger.warn(TDDatabase.TAG, this + ": Missing revision history in response from " + pathInside);
    ...
    public void insertRevisions(List<List<Object>> revs) {
        logger.info(TDDatabase.TAG, this + " inserting " + revs.size() + " revisions...");
        //logger.debug(TDDatabase.TAG, String.format("%s inserting %s", this, revs));
        ...
        try {
            for (List<Object> revAndHistory : revs) {
                ...
                if(!status.isSuccessful()) {
                    if(status.getCode() == TDStatus.FORBIDDEN) {
                        logger.info(TDDatabase.TAG, this + ": Remote rev failed validation: " + rev);
                    } else {
                        logger.warn(TDDatabase.TAG, this + " failed to write " + rev + ": status=" + status.getCode());
            ...
            logger.warn(TDDatabase.TAG, this + " finished inserting " + revs.size() + " revisions");
            success = true;
        } catch(Exception e) {
            logger.warn(TDDatabase.TAG, this + ": Exception inserting revisions", e);
        } finally {
    ...
    public String joinQuotedEscaped(List<String> strings) {
        ...
        try {
            ...
            logger.warn(TDDatabase.TAG, "Unable to serialize json", e);
        }
