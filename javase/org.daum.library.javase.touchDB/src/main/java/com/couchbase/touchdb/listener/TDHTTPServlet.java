...
public class TDHTTPServlet extends HttpServlet {
    ...
    private static Logger logger = LoggerFactory.getLogger(TDHTTPServlet.class);
    ...
    public void service(HttpServletRequest request, final HttpServletResponse response)
        ...
        logger.debug(TDDatabase.TAG, String.format("Buffer size is %d", response.getBufferSize()));
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Interrupted waiting for result", e);
        } finally {
            if(router != null) {
                router.stop();
            }
