...
public class TDPusher extends TDReplicator implements Observer {
    ...
    private  Logger logger = LoggerFactory.getLogger(TDPusher.class);
    ...
    public void maybeCreateRemoteDB() {
        ...
        logger.debug(TDDatabase.TAG, "Remote db might not exist; creating it...");
        sendAsyncRequest("PUT", "", null, new TDRemoteRequestCompletionBlock() {
            ...
            public void onCompletion(Object result, Throwable e) {
                if(e != null && e instanceof HttpResponseException && ((HttpResponseException)e).getStatusCode() != 412) {
                    logger.error(TDDatabase.TAG, "Failed to create remote db", e);
                    error = e;
                    stop();
                } else {
                    logger.debug(TDDatabase.TAG, "Created remote db");
                    createTarget = false;
                    beginReplicating();
                }
            }
    ...
    public void beginReplicating() {
        ...
        if(filterName != null && filter == null) {
            logger.warn(TDDatabase.TAG, String.format("%s: No TDFilterBlock registered for filter '%s'; ignoring", this, filterName));;
        }
        ...
        sendAsyncRequest("POST", "/_revs_diff", diffs, new TDRemoteRequestCompletionBlock() {
            ...
            public void onCompletion(Object response, Throwable e) {
                ...
                if(e != null) {
                            ...
                            if(revs != null && revs.contains(rev.getRevId())) {
                                ...
                                if(rev.isDeleted()) {
                                    ...
                                    if(!status.isSuccessful()) {
                                        logger.warn(TDDatabase.TAG, String.format("%s: Couldn't get local contents of %s", this, rev));
                                    } else {
                    ...
                    logger.info(TDDatabase.TAG, String.format("%s: Sending %d revisions", this, numDocsToSend));
                    logger.debug(TDDatabase.TAG, String.format("%s: Sending %s", this, inbox));
                    ...
                    sendAsyncRequest("POST", "/_bulk_docs", bulkDocsBody, new TDRemoteRequestCompletionBlock() {
                        ...
                        public void onCompletion(Object result, Throwable e) {
                            if(e != null) {
                                ...
                                logger.debug(TDDatabase.TAG, String.format("%s: Sent %s", this, inbox));
