...
public abstract class TDReplicator extends Observable {
    ...
    public TDReplicator(TDDatabase db, URL remote, Boolean continuous, HttpClientFactory clientFacotry) {
        ...
        batcher = new TDBatcher<TDRevision>(db.getHandler(), INBOX_CAPACITY, PROCESSOR_DELAY, new TDBatchProcessor<TDRevision>() {
            ...
            public void process(List<TDRevision> inbox) {
                logger.debug(TDDatabase.TAG, "*** " + toString() + ": BEGIN processInbox (" + inbox.size() + " sequences)");
                processInbox(new TDRevisionList(inbox));
                logger.debug(TDDatabase.TAG, "*** " + toString() + ": END processInbox (lastSequence=" + lastSequence);
                active = false;
            }
        });
    ...
    public void setLastSequence(String lastSequenceIn) {
        if(!lastSequenceIn.equals(lastSequence)) {
            logger.debug(TDDatabase.TAG, toString() + ": Setting lastSequence to " + lastSequenceIn + " from( " + lastSequence + ")");
    ...
    public void start() {
        ...
        logger.debug(TDDatabase.TAG, toString() + " STARTING ...");
        running = true;
        lastSequence = null;
    ...
    public void stop() {
        ...
        logger.debug(TDDatabase.TAG, toString() + " STOPPING...");
        batcher.flush();
        continuous = false;
        if(asyncTaskCount == 0) {
            stopped();
    ...
    public void stopped() {
        logger.debug(TDDatabase.TAG, toString() + " STOPPED");
        running = false;
    ...
    public void addToInbox(TDRevision rev) {
        ...
        //logger.debug(TDDatabase.TAG, String.format("%s: Received #%d %s", toString(), rev.getSequence(), rev.toString()));
    }
    ...
    public void sendAsyncRequest(String method, String relativePath, Object body, TDRemoteRequestCompletionBlock onCompletion) {
        //logger.debug(TDDatabase.TAG, String.format("%s: %s .%s", toString(), method, relativePath));
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Malformed URL for async request", e);
        }
    }
            ...
            public void onCompletion(Object result, Throwable e) {
                if(e != null && e instanceof HttpResponseException && ((HttpResponseException)e).getStatusCode() != 404) {
                    ...
                    if(remoteLastSequence != null && remoteLastSequence.equals(localLastSequence)) {
                        ...
                        logger.debug(TDDatabase.TAG, this + ": Replicating from lastSequence=" + lastSequence);
                    } else {
                        logger.debug(TDDatabase.TAG, this + ": lastSequence mismatch: I had " + localLastSequence + ", remote had " + remoteLastSequence);
                    }
                    beginReplicating();
                }
                asyncTaskFinished(1);
    ...
    public void saveLastSequence() {
        ...
        logger.debug(TDDatabase.TAG, this + " checkpointing sequence=" + lastSequence);
        ...
        sendAsyncRequest("PUT", "/_local/" + remoteCheckpointDocID, body, new TDRemoteRequestCompletionBlock() {
            ...
            public void onCompletion(Object result, Throwable e) {
                ...
                if(e != null) {
                    logger.debug(TDDatabase.TAG, this + ": Unable to save remote checkpoint", e);
