...
public class TDChangeTracker implements Runnable {
    ...
    public URL getChangesFeedURL() {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Changes feed ULR is malformed", e);
        }
        return result;
    }
        ...
        while (running) {
            ...
            if(url.getUserInfo() != null) {
                if(url.getUserInfo().contains(":")) {
                    ...
                    logger.warn(TDDatabase.TAG, "Unable to parse user info, not setting credentials");
                }
            }

            try {
                logger.debug(TDDatabase.TAG, "Making request to " + getChangesFeedURL().toString());
                HttpResponse response = httpClient.execute(request);
                StatusLine status = response.getStatusLine();
                if(status.getStatusCode() >= 300) {
                    logger.error(TDDatabase.TAG, "Change tracker got error " + Integer.toString(status.getStatusCode()));
                    stop();
                }
                	...
                	try {
	                    ...
	                    if(mode != TDChangeTrackerMode.Continuous) {
	                        ...
	                        if(mode == TDChangeTrackerMode.LongPoll && responseOK) {
	                            logger.debug(TDDatabase.TAG, "Starting new longpoll");
	                            continue;
	                        } else {
	                            logger.warn(TDDatabase.TAG, "Change tracker calling stop");
	                            stop();
	                        }
	                    }
	                    else {
                ...
                logger.error(TDDatabase.TAG, "ClientProtocolException in change tracker", e);
            } catch (IOException e) {
                if(running) {
                    //we get an exception when we're shutting down and have to
                    //close the socket underneath our read, ignore that
                    logger.error(TDDatabase.TAG, "IOException in change tracker", e);
                }
            }
        }
        logger.debug(TDDatabase.TAG, "Change tracker run loop exiting");
    }
        ...
        if(line.length() > 1) {
            try {
                ...
                if(!receivedChange(change)) {
                    logger.warn(TDDatabase.TAG, String.format("Received unparseable change line from server: %s", line));
                    return false;
                }
            } catch (Exception e) {
                logger.warn(TDDatabase.TAG, "Exception parsing JSON in change tracker", e);
                return false;
            }
        }
        return true;
    ...
    public void setUpstreamError(String message) {
        logger.warn(TDDatabase.TAG, String.format("Server error: %s", message));
    ...
    public void stop() {
        logger.debug(TDDatabase.TAG, "changed tracker asked to stop");
        running = false;
        thread.interrupt();
        if(request != null) {
            request.abort();
    ...
    public void stopped() {
        logger.debug(TDDatabase.TAG, "change tracker in stopped");
        if (client != null) {
            logger.debug(TDDatabase.TAG, "posting stopped");
            client.changeTrackerStopped(TDChangeTracker.this);
        }
        client = null;
        logger.debug(TDDatabase.TAG, "change tracker client should be null now");
    }
