...
public class TDRouter implements Observer {
    ...
    public Object getJSONQuery(String param) {
        ...
        try {
            ...
            logger.warn("Unable to parse JSON Query", e);
        }
        return result;
    }
    ...
    public void start() {
        ...
        if(path == null) {
            ...
            try {
                ...
                logger.error(TDDatabase.TAG, "Error closing empty output stream");
            }
            sendResponse();
            return;
        }
        ...
        logger.debug("path: " + path + " message: " + message + " docID: " + docID + " attachmentName: " + attachmentName);
        ...
        try {
            ...
            //default status is internal server error
            logger.error(TDDatabase.TAG, "Exception in TDRouter", e);
        }
        ...
        if(accept != null && !"*/*".equals(accept)) {
            ...
            if(responseType != null && accept.indexOf(responseType) < 0) {
                logger.error(TDDatabase.TAG, String.format("Error 406: Can't satisfy request Accept: %s", accept));
    ...
    public TDStatus do_POST_Document_bulk_docs(TDDatabase _db, String _docID, String _attachmentName) {
        ...
        try {
            ...
            logger.warn(TDDatabase.TAG, String.format("%s finished inserting %d revisions in bulk", this, docs.size()));
            ok = true;
        } catch (Exception e) {
            logger.warn(TDDatabase.TAG, String.format("%s: Exception inserting revisions in bulk", this), e);
        } finally {
            db.endTransaction(ok);
        }
        logger.debug(TDDatabase.TAG, "results: " + results.toString());
        ...
        try {
            ...
            if(callbackBlock != null) {
                ...
                try {
                    ...
                    logger.error(TDDatabase.TAG, "IOException writing to internal streams", e);
                }
            }
        } catch (Exception e) {
            logger.warn("Unable to serialize change to JSON", e);
        }
    }
    ...
    public void update(Observable observable, Object changeObject) {
        if(observable == db) {
            ...
            if(longpoll) {
                logger.warn(TDDatabase.TAG, "TDRouter: Sending longpoll response");
                sendResponse();
                ...
                if(callbackBlock != null) {
                    ...
                    try {
                        ...
                        logger.warn(TDDatabase.TAG, "Error serializing JSON", e);
                    }
                    OutputStream os = connection.getResponseOutputStream();
                    try {
                        os.write(data);
                ...
                logger.warn(TDDatabase.TAG, "TDRouter: Sending continous change chunk");
    ...
    public TDStatus do_GET_Document(TDDatabase _db, String docID, String _attachmentName) throws SQLException {
    ...
    public TDView compileView(String viewName, Map<String,Object> viewProps) {
        ...
        if(mapBlock == null) {
            logger.warn(TDDatabase.TAG, String.format("View %s has unknown map function: %s", viewName, mapSource));
            return null;
        }
        ...
        if(reduceSource != null) {
            ...
            if(reduceBlock == null) {
                logger.warn(TDDatabase.TAG, String.format("View %s has unknown reduce function: %s", viewName, reduceBlock));
                return null;
            }
        }
