...
import com.couchbase.touchdb.sqlite.Cursor;
import com.couchbase.touchdb.sqlite.SQLiteDatabase;
import com.couchbase.touchdb.sqlite.SQLiteException;
...
public class TDDatabase extends Observable {
    ...
    public boolean open() {
        ...
        try {
            ...
            // TODO new collation JSON
            final Collator collator_JSON      = new Collator("JSON") {
                public int compare(String str1, String str2) {
                    java.text.Collator c = java.text.Collator.getInstance();
                    int res = c.compare(str1,str2);
                    return res;
                }
            };


            final Collator collator_JSON_RAW      = new Collator("JSON_RAW") {
                public int compare(String str1, String str2) {
            ...
            final Collator collator_JSON_ASCII      = new Collator("JSON_ASCII") {
                public int compare(String str1, String str2) {
                    if (str1.length() < str2.length()) {
                        return -1;
                    } else if (str1.length() > str2.length()) {
             ...
             ((JdbcConnection)database.getConnection()).createCollationSequence(collator_JSON);
            ((JdbcConnection)database.getConnection()).createCollationSequence(collator_JSON_RAW);
            ((JdbcConnection)database.getConnection()).createCollationSequence(collator_JSON_ASCII);

        }
        catch(SQLiteException e) {
            logger.error(TDDatabase.TAG, "Error opening", e);
            return false;
        } catch (SQLException e) {
        ...
        if(!initialize("PRAGMA foreign_keys = ON;")) {
            logger.error(TDDatabase.TAG, "Error turning on foreign keys");
            return false;
        }
        ...
        if(dbVersion >= 100) {
            logger.warn(TDDatabase.TAG, "TDDatabase: Database version (" + dbVersion + ") is newer than I know how to work with");
            database.close();
            return false;
        }
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Could not initialize attachment store", e);
            database.close();
            return false;
        }
    ...
    public boolean beginTransaction() {
        try {
            ...
            ++transactionLevel;
            //logger.info(TAG, "Begin transaction (level " + Integer.toString(transactionLevel) + ")...");
        } catch (SQLException e) {
            return false;
        }
        return true;
    ...
    public boolean endTransaction(boolean commit) {
        ...
        if(commit) {
            //logger.info(TAG, "Committing transaction (level " + Integer.toString(transactionLevel) + ")...");
            database.setTransactionSuccessful();
            database.endTransaction();
        }
        else {
            logger.info(TAG, "CANCEL transaction (level " + Integer.toString(transactionLevel) + ")...");
            try {
                database.endTransaction();
            } catch (Exception e) {
                return false;
    ...
    public TDStatus compact() {
        ...
        try {
            logger.info(TDDatabase.TAG, "Deleting JSON of old revisions...");
            ContentValues args = new ContentValues();
            args.put("json", (String)null);
            database.update("revs", args, "current=0", null);
        } catch (SQLException e) {
            logger.error(TDDatabase.TAG, "Error compacting", e);
            return new TDStatus(TDStatus.INTERNAL_SERVER_ERROR);
        }

        logger.info(TDDatabase.TAG, "Deleting old attachments...");
        TDStatus result = garbageCollectAttachments();

        logger.info(TDDatabase.TAG, "Vacuuming SQLite database...");
        try {
            database.execSQL("VACUUM");
        } catch (SQLException e) {
            logger.error(TDDatabase.TAG, "Error vacuuming database", e);
            return new TDStatus(TDStatus.INTERNAL_SERVER_ERROR);
        }
    ...
    public String privateUUID() {
        ...
        try {
            ...
            logger.error(TAG, "Error querying privateUUID", e);
        } finally {
            if(cursor != null) {
                cursor.close();
            }
    ...
    public int getDocumentCount() {
        String sql = "SELECT COUNT(DISTINCT doc_id) as n FROM revs WHERE current=1 AND deleted=0";
        Cursor cursor = null;
        int result = 0;
        try {
            ...
            logger.error(TDDatabase.TAG, "Error getting document count", e);
        } finally {
            if(cursor != null) {
                cursor.close();
            }
    ...
    public long getLastSequence() {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error getting last sequence", e);
        } finally {
            if(cursor != null) {
                cursor.close();
            }
    ...
    public  byte[] appendDictToJSON(byte[] json, Map<String,Object> dict) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error convert extra JSON to bytes", e);
            return null;
        }
    ...
    public Map<String,Object> extraPropertiesForRevision(TDRevision rev, EnumSet<TDContentOptions> contentOptions) {
        ...
        if(contentOptions.contains(TDContentOptions.TDIncludeConflicts)) {
            TDRevisionList revs = null;
            revs = getAllRevisionsOfDocumentID(docId, true);
            if(revs.size() > 1) {
    ...
    public Map<String, Object> documentPropertiesFromJSON(byte[] json, String docId, String revId, long sequence, EnumSet<TDContentOptions> contentOptions) {
      ...
      try {
          ...
          logger.error(TDDatabase.TAG, "Error serializing properties to JSON", e);
      }
    ...
    public TDRevision getDocumentWithIDAndRev(String id, String rev, EnumSet<TDContentOptions> contentOptions) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error getting document with id and rev", e);
        } finally {
            if(cursor != null) {
                cursor.close();
            }
    ...
    public TDStatus loadRevisionBody(TDRevision rev, EnumSet<TDContentOptions> contentOptions) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error loading revision body", e);
            return new TDStatus(TDStatus.INTERNAL_SERVER_ERROR);
        } finally {
            if(cursor != null) {
                cursor.close();
    ...
    public long getDocNumericID(String docId) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error getting doc numeric id", e);
        } finally {
            if(cursor != null) {
                cursor.close();
            }
     ...
     */
    public TDRevisionList getAllRevisionsOfDocumentID(String docId, long docNumericID, boolean onlyCurrent) throws SQLException {
    ...
    public List<String> getConflictingRevisionIDsOfDocID(String docID) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error getting all revisions of document", e);
            return null;
        } finally {
            if(cursor != null) {
                cursor.close();
    ...
    public List<TDRevision> getRevisionHistory(TDRevision rev) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error getting revision history", e);
            return null;
        } finally {
            if(cursor != null) {
                cursor.close();
    ...
    public TDRevisionList changesSince(long lastSeq, TDChangesOptions options, TDFilterBlock filter) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error looking for changes", e);
        } finally {
            if(cursor != null) {
                cursor.close();
            }
    ...
    public List<TDView> getAllViews() {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error getting all views", e);
        } finally {
            if(cursor != null) {
                cursor.close();
            }
    ...
    public Map<String,Object> getDocsWithIDs(List<String> docIDs, TDQueryOptions options) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error getting all docs", e);
            return null;
        } finally {
            if(cursor != null) {
                cursor.close();
    ...
    public TDStatus insertAttachmentForSequenceWithNameAndType(InputStream contentStream, long sequence, String name, String contentType, int revpos) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error inserting attachment", e);
            return new TDStatus(TDStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public TDStatus copyAttachmentNamedFromSequenceToSequence(String name, long fromSeq, long toSeq) {
        ...
        try {
            ...
            if(rowsUpdated == 0) {
                ...
                logger.warn(TDDatabase.TAG, "Can't find inherited attachment " + name + " from seq# " + Long.toString(fromSeq) + " to copy to " + Long.toString(toSeq));
                return new TDStatus(TDStatus.NOT_FOUND);
            }
            else {
                return new TDStatus(TDStatus.OK);
            ...
            logger.error(TDDatabase.TAG, "Error copying attachment", e);
            return new TDStatus(TDStatus.INTERNAL_SERVER_ERROR);
        } finally {
            if(cursor != null) {
                cursor.close();
    ...
    public TDAttachment getAttachmentForSequence(long sequence, String filename, TDStatus status) {
        ...
        try {
            ...
            if(contentStream == null) {
                logger.error(TDDatabase.TAG, "Failed to load attachment");
    ...
    public Map<String,Object> getAttachmentsDictForSequenceWithContent(long sequence, boolean withContent) {
        ...
        try {
            ...
            while(!cursor.isAfterLast()) {
                ...
                if(withContent) {
                    ...
                    if(data != null) {
                        ...
                        logger.warn(TDDatabase.TAG, "Error loading attachment");
                    }
                }
            ...
            logger.error(TDDatabase.TAG, "Error getting attachments for sequence", e);
            return null;
        } finally {
            if(cursor != null) {
                cursor.close();
    ...
    public void stubOutAttachmentsIn(TDRevision rev, int minRevPos)
    {
        ...
        for (String name : attachments.keySet()) {
            ...
            if (revPos > 0 && revPos < minRevPos && (stub == null)) {
                ...
                logger.debug(TDDatabase.TAG, "Stubbed out attachment" + rev + " " + name + ": revpos" + revPos + " " + minRevPos);
            }
        }
        if (editedProperties != null)
        ...
        for (String name : newAttachments.keySet()) {
            ...
            if(newContentBase64 != null) {
                ...
                try {
                    ...
                    logger.error(TDDatabase.TAG, "IOExeption parsing base64", e);
    ...
    public TDStatus garbageCollectAttachments() {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error deleting attachments", e);
        }
        ...
        try {
            ...
            logger.info(TDDatabase.TAG, "Deleted " + numDeleted + " attachments");

            return new TDStatus(TDStatus.OK);
        } catch (SQLException e) {
            logger.error(TDDatabase.TAG, "Error finding attachment keys in use", e);
            return new TDStatus(TDStatus.INTERNAL_SERVER_ERROR);
        } finally {
            if(cursor != null) {
                cursor.close();
    ...
    public long insertDocumentID(String docId) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error inserting document id", e);
        }
        return rowId;
    }
    ...
    public byte[] encodeDocumentJSON(TDRevision rev) {
        ...
        for (String key : origProps.keySet()) {
            if(key.startsWith("_")) {
                if(!KNOWN_SPECIAL_KEYS.contains(key)) {
                    logger.error(TAG, "TDDatabase: Invalid top-level key '" + key + "' in document to be inserted");
                    return null;
                }
            } else {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error serializing " + rev + " to JSON", e);
        }
        return json;
    }
    ...
    public long insertRevision(TDRevision rev, Long docNumericID, long parentSequence, boolean current, byte[] data) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error inserting revision", e);
        }
        return rowId;
    }
    ...
    public TDRevision putRevision(TDRevision rev, String prevRevId, boolean allowConflict, TDStatus resultStatus) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error putting revision", e1);
            return null;
        } finally {
            if(cursor != null) {
                cursor.close();
    ...
    public boolean findMissingRevisions(TDRevisionList touchRevs) {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error finding missing revisions", e);
            return false;
        } finally {
            if(cursor != null) {
                cursor.close();
        ...
        try {
            ...
            if(cursor.moveToFirst()) {
                ...
                try {
                    ...
                    logger.warn(TAG, "Error parsing local doc JSON", e);
                    return null;
                }
            ...
            logger.error(TDDatabase.TAG, "Error getting local document", e);
            return null;
        } finally {
            if(cursor != null) {
                cursor.close();
