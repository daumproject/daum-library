...
public class TDRemoteRequest implements Runnable {
    ...
    public TDRemoteRequest(TDDatabase handler, HttpClientFactory clientFactory, String method, URL url, Object body, TDRemoteRequestCompletionBlock onCompletion) {
    ...
    public void run() {
        ...
        if(body != null && request instanceof HttpEntityEnclosingRequestBase) {
            ...
            try {
                ...
                logger.error(TDDatabase.TAG, "Error serializing body of request", e);
            }
        ...
        try {
            ...
            if(status.getStatusCode() >= 300) {
                logger.error(TDDatabase.TAG, "Got error " + Integer.toString(status.getStatusCode()));
                logger.error(TDDatabase.TAG, "Request was for: " + request.toString());
                logger.error(TDDatabase.TAG, "Status reason: " + status.getReasonPhrase());
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        logger.error(TDDatabase.TAG, "TDRemoteRequestCompletionBlock throw Exception", e);
                    }
                }
            }).start();
        }
