...
public class TDView {
    ...
    private static Logger logger = LoggerFactory.getLogger(TDView.class);
    ...
    public int getViewId() {
        if (viewId < 0) {
            ...
            try {
                ...
                logger.error(TDDatabase.TAG, "Error getting view id", e);
                viewId = 0;
            } finally {
                if (cursor != null) {
                    cursor.close();
    ...
    public long getLastSequenceIndexed() {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error getting last sequence indexed");
        } finally {
            if (cursor != null) {
                cursor.close();
            }
    ...
    public Boolean setMapReduceBlocks(TDViewMapBlock mapBlock,
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error setting map block", e);
            return false;
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    public void removeIndex() {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error removing index", e);
        } finally {
    ...
    public TDStatus updateIndex() {
        logger.info(TDDatabase.TAG, "Re-indexing view " + name + " ...");
        assert (mapBlock != null);
        ...
        try {
            ...
            AbstractTouchMapEmitBlock emitBlock = new AbstractTouchMapEmitBlock() {
                ...
                public void emit(Object key, Object value) {
                    ...
                    try {
                        ...
                        logger.info(TDDatabase.TAG, "    emit(" + keyJson + ", "
                                + valueJson + ")");
                        ...
                        logger.error(TDDatabase.TAG, "Error emitting", e);
            ...
            logger.info(TDDatabase.TAG, "...Finished re-indexing view " + name
            ...
            if (!result.isSuccessful()) {
                logger.warn(TDDatabase.TAG, "Failed to rebuild view " + name + ": "
                        + result.getCode());
            }
            if(db != null) {
    ...
    public Cursor resultSetWithOptions(TDQueryOptions options, TDStatus status) throws SQLException {
        if (options == null) {
        ...
        logger.info(TDDatabase.TAG, "Query " + name + ": " + sql);
    ...
    public List<Map<String, Object>> dump() {
        ...
        try {
            ...
            logger.error(TDDatabase.TAG, "Error dumping view", e);
            return null;
        } finally {
            if (cursor != null) {
                cursor.close();
    ...
    public List<Map<String, Object>> queryWithOptions(TDQueryOptions options, TDStatus status) {
        ...
        try {
            ...
            if(reduce && (reduceBlock == null) && !group) {
                logger.warn(TDDatabase.TAG, "Cannot use reduce option in view " + name + " which has no reduce block defined");
    ...
    public static double totalValues(List<Object>values) {
        ...
        for (Object object : values) {
            if(object instanceof Number) {
                ...
                logger.warn(TDDatabase.TAG, "Warning non-numeric value found in totalValues: " + object);
            }
        }
        return total;
    }
