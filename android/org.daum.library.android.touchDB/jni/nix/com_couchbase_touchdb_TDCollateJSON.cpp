...
static ValueType valueTypeOf(char c) {
	...
	case '{':
		return kObject;
	default:
		fprintf(stderr,"Unexpected character '%c' parsing JSON", c);
		return kIllegal;
	}
}
    ...
    switch (c) {
        case 'u': {
            ...
            *in += 4;
            ...
            if (uc > 127)
                fprintf(stderr,"TDCollateJSON can't correctly compare \\u%.4s", digits);
            return (char)uc;
        }
        case 'b':   return '\b';
        case 'n':   return '\n';
...
static jstring createJavaStringFromJSON(const char** in) {
	...
	*dst++ = 0; //null terminate
	start = buf;
	//fprintf(stderr,"After stripping escapes string is: %s", start);
	...
	if (result == NULL) {
		fprintf(stderr,"Failed to convert to string: start=%p, length=%u", start, length);
	}
	return result;
}
...
(JNIEnv *env, jclass cls, jobject sqliteDatabase, jint version) {
	...
	*(void **)(&sqlite3_create_collation) = dlsym(handle, "sqlite3_create_collation");
	if(!sqlite3_create_collation) {
		fprintf(stderr,"Failed to find sqlite3_create_collation: %s", dlerror());
		return;
	}
	...
	if (clazz == NULL) {
		fprintf(stderr,"Can't find android/database/sqlite/SQLiteDatabase\n");
		return;
	}
	...
	if(version < 16) {
		...
		if (offset_db_handle == NULL) {
			fprintf(stderr,"Can't find SQLiteDatabase.mNativeHandle\n");
			return;
		}
		...
		if(offset_tl == NULL) {
			fprintf(stderr,"Can't find SQLiteDatabae.mThreadSession\n");
			return;
		}
		...
		if (tl_clazz == NULL) {
			fprintf(stderr,"Can't find java/lang/ThreadLocal\n");
			return;
		}

		jmethodID get_mid = env->GetMethodID(tl_clazz, "get", "()Ljava/lang/Object;");
		if (get_mid == NULL) {
		     fprintf(stderr,"Can't find ThreadLocal.get\n");
		     return;
		}
		...
		if (sqls_clazz == NULL) {
			fprintf(stderr,"Can't find android/database/sqlite/SQLiteSession\n");
			return;
		}
		...
		if(offset_mConnectionPool == NULL) {
			fprintf(stderr,"Can't find SQLiteSession.mConnectionPool");
			return;
		}
		jobject mcp = env->GetObjectField(session, offset_mConnectionPool);
		if(mcp == NULL) {
			fprintf(stderr,"mConnectionPool was NULL");
			return;
		}

		jclass sqlcp_clazz = env->FindClass("android/database/sqlite/SQLiteConnectionPool");
		if (sqlcp_clazz == NULL) {
			fprintf(stderr,"Can't find android/database/sqlite/SQLiteConnectionPool\n");
			return;
		}
		...
		if(offset_mMainConnection == NULL) {
			fprintf(stderr,"Can't find SQLiteConnectionPool.mAvailablePrimaryConnection");
			return;
		}
		...
		if (sqlc_clazz == NULL) {
			fprintf(stderr,"Can't find android/database/sqlite/SQLiteConnection\n");
			return;
		}

		jfieldID offset_db_handle = env->GetFieldID(sqlc_clazz, "mConnectionPtr", "I");
		if(offset_db_handle == NULL) {
			fprintf(stderr,"Can't find SQLiteConnection.mConnectionPtr");
			return;
		}
	...
	fprintf(stderr,"SQLite3 handle is %d", sqliteHandle);
