 ...
 */
@DictionaryType({
        @DictionaryAttribute(name = "ipc_key", defaultValue = "", optional = false),
        @DictionaryAttribute(name = "portEvents_tcp", defaultValue = "", optional = false),
        @DictionaryAttribute(name = "portEvents_udp", defaultValue = "", optional = false)
})
@ComponentType
@Provides({ @ProvidedPort(name = "input_port", type = PortType.MESSAGE,theadStrategy = ThreadStrategy.THREAD_QUEUE)
})
@Requires({ @RequiredPort(name = "output_port", type = PortType.MESSAGE,optional = true,theadStrategy = ThreadStrategy.THREAD_QUEUE)
})
@Library(name = "JavaSE")
public class HelloWorld extends AbstractComponentType {
    ...
    public void start () {
        try
        {
           ...
           // todo
           File binary =   AetherUtil.resolveMavenArtifact4J("org.kevoree.nativeN.testcomponent_c"+getOs(), "org.kevoree.nativeN", "2.1-SNAPSHOT", "uexe", repos);
