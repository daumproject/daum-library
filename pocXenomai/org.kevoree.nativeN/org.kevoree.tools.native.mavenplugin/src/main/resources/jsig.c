...
typedef int (*sigaction_t) (int, const struct sigaction *,
			    struct sigaction *);
...
allocate_sact ()
{
  ...
  if (sact == NULL)
    {
      sact =
	(struct sigaction *) malloc ((maxsignum + 1) *
...
static sa_handler_t
call_os_signal (int sig, sa_handler_t disp, bool is_sigset)
{
  if (os_signal == NULL)
    {
      if (!is_sigset)
...
static void
save_signal_handler (int sig, sa_handler_t disp, bool is_sigset)
{
  sigset_t set;
  if (sact == NULL)
    {
...
static sa_handler_t
set_signal (int sig, sa_handler_t disp, bool is_sigset)
{
...
static int
call_os_sigaction (int sig, const struct sigaction *act,
